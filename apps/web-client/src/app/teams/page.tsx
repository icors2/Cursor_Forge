...
export default function TeamsPage() {
  ...
  const [scheduledAt, setScheduledAt] = useState("2026-10-04T17:00:00.000Z");
  const [error, setError] = useState<string | null>(null);
  const [notice, setNotice] = useState<string | null>(null);

  /** Reloads session and active-season teams. */
  ...
  /** COACH/ADMIN schedule a game (UTC scheduledAt). */
  async function handleCreateGame(event: FormEvent): Promise<void> {
    event.preventDefault();
    setError(null);
    const body: CreateGameRequest = { teamId, opponent, scheduledAt };
    try {
      ...
      {canWrite ? (
        <div className="space-y-6">
          ...
          <form onSubmit={handleCreateGame} className="space-y-3 rounded-2xl border border-emerald-900 bg-court-900/80 p-5">
            <p className="text-sm font-semibold">Schedule a game (UTC)</p>
            <select
              className="w-full rounded-lg border border-emerald-800 bg-court-950 px-3 py-2"
              value={teamId}
