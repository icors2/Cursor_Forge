...
- **UI routes:** `/` login, `/register` (PARENT/PLAYER, or COACH with invite key), `/home` hub, `/live`, `/coach` (Stat Tracking), `/volunteer`, `/calendar`, `/announcements`, `/notes`, `/teams`, `/seasons`, `/seasons/[id]/history`, `/dues`, `/theme`, `/registrations`, `/admin`.
